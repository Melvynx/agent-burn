...
pub(crate) fn missing_pricing_warnings_for_models<'a>(
    ...
    models
        ...
        .map(|model| {
            if offline {
                format!(
                    "WARN  Missing embedded pricing for {model}; cost excludes this model. Run without --offline or update agent-burn after pricing is added."
                )
            } else {
                format!(
...
mod tests {
    ...
    fn missing_pricing_warnings_mention_embedded_pricing_offline() {
        ...
        assert_eq!(
            ...
            vec![
                "WARN  Missing embedded pricing for gpt-5.2-codex; cost excludes this model. Run without --offline or update agent-burn after pricing is added.",
            ]
        );
    }
