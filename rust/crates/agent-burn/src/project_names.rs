...
fn parse_project_name(project: &str) -> String {
    ...
    if is_windows_users_path(&cleaned) {
        ...
        let segments = cleaned
            ...
            .collect::<Vec<_>>();
        if let Some(index) = segments.iter().position(|segment| *segment == "Users") {
            let project_start = if separator == '-' {
                index + 2
            } else {
                index + 3
            };
            if project_start < segments.len() {
                cleaned = segments[project_start..].join("-");
            } else if index + 2 < segments.len() {
                cleaned = segments[index + 2..].join("-");
            }
        }
...
mod tests {
    ...
    fn formats_users_project_paths_without_dropping_project_name() {
        ...
        assert_eq!(
            format_project_name("/Users/alice/agent-burn", &aliases),
            "agent-burn"
        );
        assert_eq!(
            format_project_name("/Users/alice/Development/agent-burn", &aliases),
            "agent-burn"
        );
        assert_eq!(
            format_project_name("-Users-alice-agent-burn", &aliases),
