...
Use `--agents` to read only selected sources, skipping all other harness loaders
and their provider calls. It accepts comma-separated names and may be repeated.
Without it, `summary` reads all supported sources.
...
| `--agents <names>` | Load only comma-separated harnesses, such as `codex,claude` |
