...
function getNativePackageName(platform = process.platform, arch = process.arch) {
	if (platform === 'darwin') {
		if (arch === 'arm64') {
			return 'agent-burn-darwin-arm64';
		}
		if (arch === 'x64') {
			return 'agent-burn-darwin-x64';
		}
		return undefined;
	}
	...
	if (platform === 'linux') {
		if (arch === 'arm64') {
			return 'agent-burn-linux-arm64';
		}
		if (arch === 'x64') {
			return 'agent-burn-linux-x64';
		}
		return undefined;
	}
	...
	if (platform === 'win32') {
		if (arch === 'arm64') {
			return 'agent-burn-win32-arm64';
		}
		if (arch === 'x64') {
			return 'agent-burn-win32-x64';
		}
	}
...
function getNativeBinarySubpath(platform = process.platform) {
	return platform === 'win32' ? 'bin/agent-burn.exe' : 'bin/agent-burn';
}
...
function resolveCliRuntime({
	...
	return {
		errorMessage: `agent-burn native binary is not available for ${platform}-${arch}. Reinstall agent-burn so optional native dependencies are installed.\n`,
	};
}
...
function ensureNativeBinaryExecutable({
	...
	try {
		...
		return `agent-burn native binary is not executable: ${errorMessage(error)}\n`;
	}
}
