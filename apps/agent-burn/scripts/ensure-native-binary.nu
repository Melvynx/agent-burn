...
def main [] {
    ...
    let binary_name = if $target_platform == 'win32' { 'agent-burn.exe' } else { 'agent-burn' }
    ...
    ^cargo build --manifest-path ($repo_root | path join rust Cargo.toml) --release --bin agent-burn
...
def matching_native_package_root [repo_root: path, target_platform: string, target_arch: string] {
    let candidates = (glob ($repo_root | path join packages 'agent-burn-*' package.json) | each {|package_json_path|
...
def expected_version [repo_root: path] {
    let package_json = (open ($repo_root | path join apps agent-burn package.json))
    if (($package_json.version? | describe) != 'string') {
        error make {msg: 'apps/agent-burn/package.json version is not configured'}
    }
